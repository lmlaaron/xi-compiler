...
public class OptimalCodeWriter extends CodeWriter {
    /**
     * Create a OptimalCodeWriter object with output stream {@code o} and width
     * {@code width_}.
     * 
     * @param o
     ...
     * <p>
     * A block is a formatting unit. The formatting algorithm will try to put the
     * whole block in one line unless
     * <ul>
     * <li>there is a level-0 break in the block, or</li>
     * <li>the block cannot fit in one line.</li>
     * </ul>
     * <p>
     * If either of the two conditions is satisfied, the formatting algorithm will
     * break the block into lines by generating newlines for some of the inserted
     * breaks. The first line is printed at the current cursor position {@code pos},
     * all the following lines are printed at the position {@code pos+n}.
     *
     * @param n
     *            the number of characters increased on indentation (relative to the
     *            current position) for all lines in the block. Requires: n >= 0.
     */
    @Override
     ...
     * {@code begin}'s are closed and the current indentation level is reset to 0.
     * Returns true if formatting was completely successful (the margins were
     * obeyed).
     */
    @Override
    ...
    /**
     * Like {@code flush}, but passing {@code format=false} causes output to be
     * generated in the fastest way possible, with all breaks broken.
     * 
     * @param format
    ...
    public boolean flush(boolean format) throws IOException {
        ...
        if (format)
            ...
            brkAssignment = Collections.emptyMap();
...
class ConsList<T> {
    ...
    int length() {
        if (length == -1)
            length = next == null ? 0 : 1 + next.length();
        return length;
    }
...
class SearchState implements Cloneable {
    ...
    SearchState(int lmargin, int rmargin, int pos, int minbr, int minbu, int minbo, int maxbr, int maxbi) {
...
/**
 * An {@code OCItem} is a piece of input handed to the formatter. It contains a
 * reference to a possibly empty list of items that follow it.
 */
abstract class OCItem {
    ...
    /**
     * Try to format a whole sequence of items in the manner of formatN. Unlike for
     * formatN, The initial position may be an overrun (this is the only way that
     * overruns are checked!). The item {@code it} may be also null, signifying an
     * empty list. Requires: lmargin &lt; rmargin, pos &le; rmargin, lmargin &ge; 0.
     *
     * @see formatN
     */
    static Map<AllowBreak, Boolean> format(OCItem it, int rmargin) {
        ...
        for (OCItem cur = it; cur != null;) {
            ...
            if (OptimalCodeWriter.debug) {
                ...
                System.err.println(
                        "Format: " + cur + "\n  lmargin = " + s.lmargin + " pos = " + s.pos + " max break levels: "
                                + s.maxbr + "/" + s.maxbi + " min break levels: " + s.minbr + "/" + s.minbu);
     ...
     * @param fin
     *            is a bound on the final cursor position after formatting the whole
     *            item.
     * @param maxLevel
     *            is the maximum level at which breaks may be broken in the current
     *            block.
     * @param maxLevelInner
     ...
     *
     *            <p>
     *            Breaks may be broken up to level maxLevel, which is set whenever a
     *            break is not broken. Not breaking an ordinary break means that
     *            equal or higher-level breaks in all contained blocks must not be
     *            broken either, and breaks of strictly higher level in the same
     *            block must not be broken. Not breaking a unified break means that
     *            breaks of the same level in the same block must not also be
     *            broken. The parameter maxLevelInner controls the maxLevel in
     ...
     *            maxLevel is set to 2 and maxLevelInner is set to 1. This permits
     *            further breaks of level 1 or 2 in the same block, but only level-1
     *            breaks in inner blocks.
     *
     *            <dt>Example 2:</dt>
     *            <dd>Suppose we have a current maxLevel of 4, and a unified break
     *            of level 2 is not broken. Then for that block, maxLevel and
     *            maxLevelInner are set to 1. This permits no breaks in this block
     *            or in any nested blocks.</dd>
     *            </dl>
     *
     *            <p>
     *            When a break is broken in a nested block, it means that all equal
     *            or higher-level breaks in containing blocks must be broken.
     *            However, these breaks may be encountered after the nested block.
     *            The parameter {@code minLevel} is used to communicate the level of
     *            breaks broken in nested blocks (and earlier in the current block).
     *            Any break of level <= minLevel <em>must</em> be broken. The
     *            parameter {@code minLevelUnified} is the minimum level at which
     *            unified breaks must be broken. minLevelUnified is equal to either
     *            minLevel or minLevel+1.
     *            </p>
     *
     *            <dl>
     *            <dt>Example 3:
     *            <dd>Suppose we have a current maxLevel of 4, and a break of level
     *            2 is broken. Then for its block, minLevel is at least 1, and
     *            minLevelUnified is at least 2. For containing blocks, minLevel is
     *            at least 2.</dd>
     *            </dl>
     *
     *            <b>Note: </b> It is important that formatN not necessarily convert
     *            overruns in its final position into exceptions. This allows the
     *            calling routine to distinguish between 'internal' overruns and
     *            ones that it can tack on a conservative estimate of how much
     *            formatting the rest of the list will make the overrun go up by.
     *            Also, it simplifies the coding of formatN.
     *            </p>
     *
    ...
    /**
     * Send the output associated with this item to {@code out}, using the given
     * break settings.
     *
     * @param success
     */
    abstract int sendOutput(PrintWriter out, int lmargin, int pos, Map<AllowBreak, Boolean> brkAssignment);
    ...
    /**
     * Returns an integer array of length at least 3 such that - [0] is the minimum
     * break level that any break in the containing block of the containing block of
     * this item must break - [1] is the minimum break level that any break in the
     * containing block of this item must break - [2] is the minimum break level
     * that any unified break in the containing block of this item must break
     */
...
class AllowBreak extends OCItem {
    ...
    Map<Integer, Map<Integer, Map<Integer, Pair<Integer, ConsList<Boolean>>>>> cache = new HashMap<>();
    ...
    void selfFormat(SearchState s) {
        ...
        if (s.forward) {
            ...
            } else if (canLeaveUnbroken(s.minbr, s.minbu) && s.pos + altlen <= s.rmargin) {
            ...
            if (!s.brkAssignment.get(this) && canBreak(s.maxbr)) {
                ...
                if (afterBrkAssignment != null) {
                    ...
                    if (afterBrkAssignment.length() > s.afterBrkAssignment.length()
                            || afterBrkAssignment.length() == s.afterBrkAssignment.length() && minovf <= s.minovf) {
        ...
        if (backtrack) {
            ...
            if (assignment) {
                ...
                if (s.minbr < level)
                    s.minbr = level - 1;
                // If this is a unified break, all unified breaks of our level must also be
                // broken.
                if (unified && s.minbu < level)
                    s.minbu = level;
...
class BlockItem extends OCItem {
    ...
    /**
     * Returns an integer array of length 5 such that - [0] is the minimum break
     * level that any break in the containing block of the containing block of this
     * item must break - [1] is the minimum break level that any break in the
     * containing block of this item must break - [2] is the minimum break level
     * that any unified break in the containing block of this item must break - [3]
     * is the minimum break level that any break in this block must break - [4] is
     * the minimum break level that any unified break in this block must break
     */
    @Override
    int[] minBreakLevels() {
        if (minBreakLevels == null) {
            ...
            for (OCItem it = first; it != null; it = it.next) {
                ...
                if (minBreakLevels[2] < mbls[0])
                    minBreakLevels[2] = mbls[0];
                if (minBreakLevels[3] < mbls[1])
                    minBreakLevels[3] = mbls[1];
                if (minBreakLevels[4] < mbls[2])
                    minBreakLevels[4] = mbls[2];
            }
