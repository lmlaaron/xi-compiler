...
public class New extends Expr {
	...
	public IRNode translate() {
		...
		//String objName = "_I__obj__"+objClass.classId.getId().replace("_", "__")+"_"+ labelNumber;
		String objName = objClass.classId.getId().replace("_", "__")+"_"+ labelNumber;
		
		// 1. check the size of _I_size_someClass
		// if _I_size_Point neq zero, initialize the variable
		//stmts.add(new IRCJump(
		//		new IRTemp("_I_size_"+objClass.classId.getId().replace("_", "__")),
		//		objName+"_noinit",
		//		null));
		
		//and call _I_init_someClass to construct _I_vt_someClass
		//stmts.add(new IRExp(
		//		new IRCall(
		//				new IRName("_I_init_"+objClass.classId.getId().replace("_", "__")))));
		...
		// 3. point the DV pointer to _I_vt_someClass
		stmts.add(
				new IRMove(
						new IRMem( new IRTemp(objName)),
						new IRTemp("_I_vt_"+objClass.classId.getId().replace("_", "__"))));
