...
public class Plus extends ArithmeticOperator {
    ...
    public IRNode translateWithOperands(IRExpr... operands) {
        if (addingArray) {
            ...
            IRBinOp sizePlusOne = new IRBinOp(OpType.ADD, new IRBinOp(OpType.MUL, size, new IRConst(8)),
                    new IRConst(8));
    ...
    public void copyArray(List<IRStmt> stmts, int i, IRTemp oldArraySize) {
        ...
        IRMem memTo = new IRMem(new IRBinOp(OpType.ADD, newArray, new IRBinOp(OpType.MUL, curIndex, new IRConst(8))));
        IRMem memFrom = new IRMem(
                new IRBinOp(OpType.ADD, (i == 1 ? a1 : a2), new IRBinOp(OpType.MUL, curA1Index, new IRConst(8))));
