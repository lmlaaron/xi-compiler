...
public class HelperFunctions {
    ...
    /**
     * Generate a file from an unformatted pdf version, but doesn't guarantee
     * correctness. In rare circumstances the result needs hand correction.
     * 
     * @param source
     *            path to the source file (rough copy paste from the pdf file)
     * @param dest
     *            path to the dest file (the .xi file)
     * @param lineWidth
     *            how wide is the line markers (e.g. if there are > 100 lines, then
     *            3, > 10 lines then 2, etc)
     * @throws IOException
     */
    public static void generateFile(String source, String dest, int lineWidth) throws IOException {
        ...
        for (int i = 0; i < input.length(); i++) {
            String lineNum;
            if (i < input.length() - lineWidth + 1)
                lineNum = input.substring(i, i + lineWidth).trim();
            else
                lineNum = "";
            try {
